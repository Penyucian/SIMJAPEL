...
class Template {
    ...
    public $_ci;
    protected $brand_name = 'Sys Framework';
    ...
    public function load_view($view, $data = array(), $return = FALSE) {
        if ($this->_ci->input->is_ajax_request()) {
            ...
            foreach ($this->js as $js_file) {
                $js[] = '<script src="' . base_url() . 'sys-assets/js/' . $js_file . '"></script>';
            }
            $js = implode('', $js);

            $css = array();
            foreach ($this->css as $css_file) {
                $css[] = '<link type="text/css" rel="stylesheet" href="' . base_url() . 'sys-assets/css/' . $css_file . '"/>';
            }
            $css = implode('', $css);
