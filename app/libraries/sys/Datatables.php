...
class Datatables
{
	...
	public function order_by($key_condition, $val = 'asc')
	{
	...
	private function produce_output($output, $charset)
	{
		...
		foreach ($aaData as $num => $vd) {
			...
			if ($this->action) {
				foreach ($this->action as $v) {
					$key = explode(',', $v['key']);
					$k0 = !empty($key[0]) ? '/' . $this->ci->encryption_lib->urlencode($vd[trim($key[0])]) : null;
					$k1 = !empty($key[1]) ? '/' . $this->ci->encryption_lib->urlencode($vd[trim($key[1])]) : null;
					$k2 = !empty($key[2]) ? '/' . $this->ci->encryption_lib->urlencode($vd[trim($key[2])]) : null;
					$k3 = !empty($key[3]) ? '/' . $this->ci->encryption_lib->urlencode($vd[trim($key[3])]) : null;
					$k4 = !empty($key[4]) ? '/' . $this->ci->encryption_lib->urlencode($vd[trim($key[4])]) : null;
					$x0 = !empty($key[0]) ? $vd[trim($key[0])] : null;
					$x1 = !empty($key[1]) ? '_' . $vd[trim($key[1])] : null;
					$x2 = !empty($key[2]) ? '_' . $vd[trim($key[2])] : null;
					$x3 = !empty($key[3]) ? '_' . $vd[trim($key[3])] : null;
					if ($v['action'] == 'edit') {
						$icon = !empty($v['icon']) ? $v['icon'] : 'pencil';
						$type = !empty($v['type']) ? $v['type'] : 'warning';
						$title = !empty($v['title']) ? $v['title'] : null;
						$vd['action'] .= "<a rel='async' ajaxify='" . site_url('sys/modal/set_modal/' . $title . '/' . $v['url'] . $k0 . $k1 . $k2 . $k3 . $k4) . "' class='btn btn-" . $type . " btn-sm edit' data-toggle='tooltip' data-placement='top' title='" . $title . "'><span class='fa fa-" . $icon . "'></span></a> ";
					}
					if ($v['action'] == 'edit_full_modal') {
						$icon = !empty($v['icon']) ? $v['icon'] : 'pencil';
						$type = !empty($v['type']) ? $v['type'] : 'warning';
						$title = !empty($v['title']) ? $v['title'] : null;
						$vd['action'] .= "<a rel='async' ajaxify='" . site_url('sys/modal/set_modal_full/' . $title . '/' . $v['url'] . $k0 . $k1 . $k2 . $k3 . $k4) . "' class='btn btn-" . $type . " btn-sm edit' data-toggle='tooltip' data-placement='top' title='" . $title . "'><span class='fa fa-" . $icon . "'></span></a> ";
					}
					if ($v['action'] == 'edit_non_modal') {
						$icon = !empty($v['icon']) ? $v['icon'] : 'pencil';
						$type = !empty($v['type']) ? $v['type'] : 'warning';
						$title = !empty($v['title']) ? $v['title'] : null;
						$vd['action'] .= "<a href='" . site_url($v['url']) . $k0 . $k1 . $k2 . $k3 . "' class='btn btn-" . $type . " btn-sm edit_non_modal xhr dest_subcontent-element' data-toggle='tooltip' data-placement='top' title='" . $title . "'><span class=' icon fa fa-" . $icon . "'></span></a> ";
					}
					if ($v['action'] == 'delete') {
						$icon = !empty($v['icon']) ? $v['icon'] : 'trash-o';
						$title = !empty($v['title']) ? $v['title'] : 'Hapus';
						$vd['action'] .= "<a action='" . site_url($v['url'] . $k0 . $k1 . $k2 . $k3) . "' class='btn btn-danger btn-sm confirm-remove' data-toggle='tooltip' data-placement='top' title='" . $title . "'><span class='fa fa-" . $icon . "'></span></a> ";
					}
					if ($v['action'] == 'check') {
						$title = !empty($v['title']) ? $v['title'] : null;
						$vd['action'] .= "<label class='custom-control custom-checkbox' >
                                                            <input name='check[]' type='checkbox' class='custom-control-input cid' value='" . $x0 . $x1 . $x2 . $x3 . "' did='" . $x0 . $x1 . $x2 . $x3 . "'>
                                                            <span class='custom-control-indicator'></span>&nbsp;
                                                        </label> ";
					}
					if ($v['action'] == 'detail') {
						$title = !empty($v['title']) ? $v['title'] : null;
						$vd['action'] .= "<a href='" . site_url($v['url'] . $k0 . $k1 . $k2 . $k3) . "' class='btn btn-success btn-sm generate' data-toggle='tooltip' data-placement='top' title='" . $title . "'><span class='icon fa fa-list'></span></a> ";
					}
				}
			}
			$dataNumber[] = $vd;
		...
		if ($output == 'json') {
			$sOutput = array(
				...
				'csrf_value' => $this->ci->security->get_csrf_hash(),
			);
