...
class Ref_jabatan extends SYS_Controller
{
    private $data;
    ...
    public function __construct()
    {
        ...
        $this->data['controller'] = 'manajemen_data/ref_jabatan/';
    }
    ...
    {
        echo $this->model_ref_jabatan->get_data($this->data);
    }

    public function delete($id)
    {
        $response = $this->model_ref_jabatan->delete_data(['id' => $this->encryption_lib->urldecode($id)]);
        if ($response) {
    ...
    {
        $this->form_validation->set_rules('dt[name]', 'Jabatan', 'required|trim');
        if ($this->input->server('REQUEST_METHOD') == 'POST' && $this->form_validation->run($this) === TRUE) {
            $this->model_ref_jabatan->dbSimrs->trans_begin();
            $data = $this->input->post();

            if ($id) {
                $type = 'Update ';
                $this->model_ref_jabatan->update_data($data['dt'], ['id' => $this->encryption_lib->urldecode($id)]);
            } else {
                $type = 'Simpan ';
                $this->model_ref_jabatan->insert_data($data['dt']);
            }
            if ($this->model_ref_jabatan->dbSimrs->trans_status() === true) {
                $this->model_ref_jabatan->dbSimrs->trans_commit();
                $this->ret_css_status = 'success';
                $this->ret_message = $type . 'Data Referensi Jabatan berhasil';
                $this->ret_url = site_url($this->data['controller'] . 'view');
            } else {
                $this->model_ref_jabatan->dbSimrs->trans_rollback();
                $this->ret_css_status = 'danger';
                $this->ret_message = $type . 'Data Referensi Jabatan gagal';
            ...
            if ($id) {
                $this->data['content'] = $this->model_ref_jabatan->get_row(['id' => $this->encryption_lib->urldecode($id)])->row_array();
            }
